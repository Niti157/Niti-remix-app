...
export default function BookEditForm(){
    ...
    useEffect(() => {
        try {
            const fetchData = async () => {
                const data = await fetch(`http://localhost:3001/api/getBookById/${bookId}`);
                if (data.ok) {
    ...
    const handleSubmit = async (e) => {
      ...
      if(confirm('ยืนยันการแก้ไขข้อมูล?')){
        ...
        try {
            const response = await fetch('http://localhost:3001/api/updateBook', {
                method: 'POST',
                headers: {
    ...
    return (
        <div className="m-3">
            <h1 className="font-bold">แก้ไขข้อมูลหนังสือ</h1>
